...
`WindowsDisconnectedDeviceService` は SetupAPI から接続中 ID 集合とインストール済みデバイスを列挙し、その差から保護対象のソフトウェア／仮想デバイスを除いてUIへ返す。削除時は対象が再接続されていないか再列挙し、未接続のままならNewDevの `DiUninstallDevice` を呼ぶ。結果は削除済み、既に不在、再接続、保護対象、失敗と再起動要否に正規化され、ViewModelが再列挙後の一覧と集計表示へ反映する。
...
- 未接続デバイスは削除直前にも接続状態を確認し、再生成性が不明なデバイス種別を対象にしない。
