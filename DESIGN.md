...
利用者セッションで行うShell操作は`UnelevatedCommandExecutor`へ分離する。サービス操作は`WindowsServiceControl`に集約し、元から実行中だったサービスだけを停止・再開する。開始コマンドには2分の上限を設け、失敗時もSCMによる自動回復を最大60秒待って実稼働状態を確認してから再開失敗を確定する。
...
| サインイン時クリーンアップを信頼済みPerMachine実体の最高権限タスクに限定する | UACなしでサービス停止を伴う掃除を可能にしつつ、ユーザー書込み可能な実体への差し替えを防ぐ | 手動コピーや開発ビルドからはタスクを登録できない |
