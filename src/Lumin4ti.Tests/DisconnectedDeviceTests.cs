...
public sealed class DisconnectedDeviceTests
{
    ...
    public void 再インストール性が不明なソフトウェアデバイスは保護する(string instanceId)
    {
        Assert.IsTrue(WindowsDisconnectedDeviceService.IsProtectedSoftwareDevice(instanceId));
        Assert.IsFalse(WindowsDisconnectedDeviceService.IsDisconnectedCandidate(instanceId, isPresent: false));
    }

    [TestMethod]
    public void 未接続の通常PnPデバイスだけを候補にする()
    {
        const string instanceId = @"USB\VID_1234&PID_5678\TEST";

        Assert.IsFalse(WindowsDisconnectedDeviceService.IsProtectedSoftwareDevice(instanceId));
