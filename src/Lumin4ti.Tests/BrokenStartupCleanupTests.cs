...
public sealed class BrokenStartupCleanupTests
{
    ...
    [TestMethod]
    public void 削除対象は欠損を確定できるRun登録だけに限定する()
    {
        var missing = BrokenStartupCleanupAction.GetConfirmedMissingExecutable(
            @"""C:\Tools\gone.exe"" --silent",
            path => path.EndsWith("gone.exe", StringComparison.OrdinalIgnoreCase));
        var existing = BrokenStartupCleanupAction.GetConfirmedMissingExecutable(
            @"C:\Tools\app.exe",
            _ => false);
        var unresolved = BrokenStartupCleanupAction.GetConfirmedMissingExecutable(
            @"app.exe --silent",
            _ => true);

        Assert.AreEqual(@"C:\Tools\gone.exe", missing);
