...
/// <summary>
/// エクスプローラーのクイックアクセスにピン留めしたフォルダをパスの昇順に並べ替える。
...
public sealed class QuickAccessSortAction : IMaintenanceAction
{
    ...
    public string Description => "クイックアクセスにピン留めしたフォルダをパスの昇順に並べ替えます (実行前にジャンプリストをバックアップします)。";
    ...
    internal static string PowerShellScript =>
        ...
        function Invoke-QuickAccessSort {
            ...
            $sorted = @($pinnedItems |
                Sort-Object @{ Expression = { [string]$_.Path }; Ascending = $true } |
